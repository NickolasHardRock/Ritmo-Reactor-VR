...
depois do fim; nunca durante. Com o card de HTML (`?menu2d=1`) o envio espera
o jogador confirmar o nome — ver `enviarResultadoUmaVez` em
`frontend/src/api.js` e o campo `#fim-nome` em `frontend/index.html`. No fluxo
normal quem decide o momento é `frontend/src/registro.js`: envia na hora,
a menos que a partida tenha batido o recorde (RN09), quando espera o nome.

A resposta traz **`recorde`** — se esta partida virou a nova melhor marca
daquela música naquele nível. **Quem decide é o servidor**, nunca o corpo
enviado: o jogo roda na máquina do jogador e um campo vindo dali seria só uma
sugestão. A consulta acontece **antes** do INSERT; depois, a própria partida
já seria a marca a bater e nada nunca seria recorde. Regra em
`backend/regras.js`.
...
{ "id": 1, "nome": "Diego", "pontos": 740, "tempo": 96.2,
  "estrelas": 4, "recorde": true, "recordeAnterior": null,
...
A **marca a bater** numa música e nível. O jogo consulta no fim de toda
partida concluída para decidir se para e pede o nome de quem jogou (RN09).
...
{ "musica": "colour-me-red", "nivel": "facil",
  "recorde": { "posicao": 1, "nome": "Diego", "pontos": 7400, "tempo": 96.2,
               "precisao": 88, "combo_max": 17, "estrelas": 4,
               "criado": "2026-09-01T23:16:02.194Z" },
  "minimo": 1000 }
```

`"recorde": null` quer dizer que **ninguém jogou essa música nesse nível
ainda** — e é diferente de zero ponto: a primeira partida da tabela também é
recorde, desde que alcance `minimo`.

Por dentro é o mesmo `ranking(1, {musica, nivel})` que alimenta o recorde da
tela de escolha — **de propósito**: uma segunda consulta com ordenação própria
seria uma segunda definição de "melhor", esperando divergir da primeira.

Esta rota existe separada do `GET /api/ranking?limite=1`, que devolveria a
mesma linha, por causa do **`minimo`**: é ele que fecha a regra da RN09, e ele
é do servidor. Sem isso o jogo precisaria de uma cópia do piso — e regra de
negócio duplicada é regra que diverge, ainda por cima com a cópia morando no
navegador, onde qualquer um edita. O jogo **não mostra esse número ao
jogador**: é critério da equipe, não meta dele.

**400** sem `musica` ou sem `nivel`, ou com qualquer um dos dois mal formado.
